...
func setupRouting(webSrv server.Server, boxManager box.Manager, authManager auth.Manager, rtConfig config.Config) {
	...
	webSrv.AddListRoute('w', server.MethodGet, a.MakeListPlainHandler(ucListMeta))
	webSrv.AddZettelRoute('w', server.MethodGet, a.MakeGetPlainZettelHandler(ucGetZettel))
