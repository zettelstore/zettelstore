...
func (cp *constBox) SelectMeta(ctx context.Context, match search.MetaMatchFunc) (sel, rej box.MetaMap, err error) {
	sel = make(box.MetaMap, len(cp.zettel))
	rej = make(box.MetaMap, len(cp.zettel))
