...
func TestSimple(t *testing.T) {
	...
	if zid != id.ZidO(1) || action != arZettel || lastReload {
	...
	ar.EnqueueZettel(id.ZidO(1))
	ar.EnqueueZettel(id.ZidO(2))
	if ar.first != ar.last {
		t.Errorf("Expected one room, but got more")
	}
	ar.EnqueueZettel(id.ZidO(3))
	if ar.first == ar.last {
...
func TestReset(t *testing.T) {
	...
	if !(zid1 == id.ZidO(3) && zid2 == id.ZidO(4) || zid1 == id.ZidO(4) && zid2 == id.ZidO(3)) {
		t.Errorf("Zids must be 3 or 4, but got %v/%v", zid1, zid2)
	}
	action, zid, _ = ar.Dequeue()
	if zid != id.ZidO(5) || action != arZettel {
	...
	ar = newAnteroomQueue(1)
	ar.Reload(id.NewSetO(id.ZidO(6)))
	action, zid, _ = ar.Dequeue()
	if zid != id.ZidO(6) || action != arZettel {
